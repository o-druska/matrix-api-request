...
def create_parser() -> argparse.ArgumentParser:
    ...
    login.add_argument('-l', '--login',
                       ...
                       nargs=2, type=str)
    login.add_argument('-d', '--dummy',
                       help="Makes a dummy call to the API login; API will return a dummy access token",
...
def make_request_call(request_method: str, url: str, headers: dict) -> dict:
    response = rq.request(method=request_method.capitalize(),
                          url=url,
                          headers=headers)
...
def main() -> None:
    ...
    print("args:\n" + str(args))
    print("Server: " + str(matrix_server))

    if args.dummy:
        args.token = get_access_token(_login_type="m.login.dummy",
